...
pub fn read_message_from_reader(reader: &mut Read, delimiter_string: String, ending_boundary: String, beginning_boundary: String) -> Option<Vec<u8>> {
        ...
        if beg_bound_sized_vec.as_slice() == beginning_boundary.as_bytes() {
            ...
            while !contains_delimiter {
                ...
                if acc_buff.ends_with(delimiter_string.as_bytes()) {
                    // we have found the delimiter, end the loop at next iter
                    ...
                    if let Some(v) = find_where_slice_begins(&acc_buff, delimiter_string.as_bytes()) {
                        ...
                        if let Ok(buf_str) = String::from_utf8(acc_buff.clone()) {
                            if let Ok(num) = str::parse::<usize>(&buf_str) {
